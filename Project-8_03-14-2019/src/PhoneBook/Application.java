...
public class Application {
	...
	private static String[] menus = {"Add new entries","Search by first name *","Search by full name *",
			...
			"Exit"};
	private static String[] personMenus = {"Update Name","Update Middle Name","Update Last Name","Update Address","Update Contact", "Update Birthday Exp(Month-Day-Year)", "Update Aniverasry Exp(Month-Day-Year)"
			,"Exit"};
	
	private static String[] addressMenus = {"Update Street","Update City","Update State","Update Zipcode",
	"Exit"};
	
	private static String[] contactMenus = {"Update Telephone","Update Home Number","Update Cell","Update Email","Update Fax",
	"Exit"};
	...
	public static void runningType(Scanner sc, String menuType,Person person) {
		...
		//This happens only for person menu as decoration to see which user being update by their name
		...
		while(option != mymenus.length) {
			...
			//This function will take response, menutype,Person object in order to redirect to use* right method
	 ...
	 * option
	 * Using switch statement which makes it easy instead of bunch of if statements
	 */	
	public static String Option(int option,String menuType) {
		String result = "Please enter to ";
		
		
		//Based on the menus it will add resul string based on the option if it is over 8 it will return default string which says enter valid option
		if(menuType == "mainMenu") {
			switch(option) {
			case 1 : return result += "add new entry exp(John Doe, 114 Market St, St Louis, MO, 63403, 6366435698): ";
		...
		if(menuType == "personMenus") {
			...
			case 6 : return result += "Update Birthday Exp(Month-Day-Year): ";
			case 7 : return result += "Update Aniverasry Exp(Month-Day-Year): ";
			default : return "Please enter only from an options!";
			}
		}
		...
		if(menuType == "contactMenus") {
			...
			case 2 : return result += "Update Home Number: ";
			case 3 : return result += "Update Cell: ";
			case 4 : return result += "Update Email: ";
			case 5: return result += "Update Fax: ";
			default : return "Please enter only from an options!";
			}
		}
	 ...
	 * to use* right method for exp if menuType is main Main it will get right method to use based on the which option I used
	 */	
	public static void redirect(int option, String response,String menuType,Person person) {
		...
		//Ones I get person based on their telephone number, updating person like person.setFirstName if option is 1 samething for rest of it
		...
		if(menuType == "personMenus") {
			...
			case 6 : updatePerson(response,option,person);break;
			case 7 : updatePerson(response,option,person);break;
			default : break;
			}
		...
		//From personMenus I am getting person.getAddress class and set function (UPDATING IT)
		if(menuType == "addressMenus") {
		...
		//From personMenus I am getting person.Contact class and set function (UPDATING IT)
		if(menuType == "contactMenus") {
			
			switch(option) {
			case 1 : updateContact(response,option,person); break;
			case 2 : updateContact(response,option,person); break;
			case 3 : updateContact(response,option,person); break;
			case 4 : updateContact(response,option,person); break;
			case 5 : updateContact(response,option,person); break;
			default : break;
			}
	...
	/*
	 * This function direct to continuous redirect with Person object to runningType function both updateAddress and updateContactScreen method
	 */	
	...
	public static void updatePerson(String response,int option, Person person) {
		...
		if(option == 6) person.setBirthday(response);
		if(option == 7) person.setAniversary(response);
	...
	public static void updateContact(String response,int option, Person person) {
		...
		if(option == 1) person.getContact().setTelephone(onlyNumber);
		if(option == 2) person.getContact().setHome(onlyNumber);
		if(option == 3) person.getContact().setCell(onlyNumber);
		if(option == 4) person.getContact().setEmail(response);
		if(option == 5) person.getContact().setFax(onlyNumber);
	...
	public static void addEntry(String response) {
		...
		//This will extract raw string first name, middle name if it exist and last name and assigns to new Person
	...
	public static void search(String response, int opt) {
		...
		//This will sort serachList in ASC  please go to sortPersonList method I have explain it in there
